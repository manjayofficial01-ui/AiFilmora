...
vec4 FUNCNAME(vec2 tc)
{
  if(PREFIX(uStep) == 0){
    ...
    vec4 shading = clamp(relit(oriCol, tc) * 1.0, 0.0, 1.0);
    return shading;
    // return vec4(GetNormalMap(tc), 1.0);
